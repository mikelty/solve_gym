'''
some parameters might still need adjustment
expected sarsa works best
TODO plot everything
thought: combine sarsa with double q. don't know if that's good for a particularly nasty problem or not...
'''
import numpy as np
...
def td0_eval(p,num_iter=10_000,alpha=0.1,gamma=0.9):
...
v=td0_eval(p)
print(v.reshape(int(env.nS**.5),-1))
'''
def SARSA(num_iter=50_000,alpha=0.01,gamma=0.9,eps=0.1,expected=True,lam=0.6):
    '''
    ...
    for i in tqdm(range(num_iter),desc=f'SARSA (alpha: {alpha}, gamma: {gamma}, epsilon: {eps}, expected: {expected}, lambda: {lam})',unit='eps'):
        ...
        while not done:
            ...
            if expected:
                q_=sum(q[s_,a_]*eps/env.nA for a_ in range(env.nA))+(1-eps)*q[s_,np.argmax(q[s_])]
            else:
...
p,q=SARSA(expected=False,lam=None)
#print(f'p:\n{p}')
#print(f'q:\n{q}')
print(test(p))
...
def q_learning(num_iter=50_000,alpha=0.01,gamma=0.9,eps=0.1,double_q=True):
    '''
